...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [(JFImagePickerController *)self.navigationController setLeftTitle:@"Cancel"];
...
- (void)viewWillDisappear:(BOOL)animated{
	...
	if (!isBrowser) {
		if (ASSETHELPER.selectdPhotos.count>0) {
			[(JFImagePickerController *)self.navigationController setLeftTitle:@"Preview"];
		} else {
	...
	if (!isPreview) {
		if ([placeholder.backgroundColor isEqual:[[UIColor blackColor] colorWithAlphaComponent:.1]]) {
			if (ASSETHELPER.selectdPhotos.count>= ASSETHELPER.maxCount) {
                [[[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"you may choose most %@ photos",@(ASSETHELPER.maxCount)] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
				return;
			}
